...
The purpose of this repository is to host performance tests, benchmarks, and prototypes, as well as serve as the main forum for an open initiative to greatly improve the performance of storing and retrieving data from common database systems using common .NET data access APIs.

## Background

While we have achieved great gains in other areas, database access benchmarks show that .NET Core still lags significantly behind other platforms.
...
The performance of accessing databases using .NET should be much more competitive, and we (several people from Microsoft and the .NET developer community) have decided to do something about it.
...
- __Everything is on the table right now:__
  ...
  - We need to assume we don't know much about what makes a data access APIs really fast (otherwise we wouldn't need to do this work, right?). For this reason we need to keep an open mind when looking at alternative lines of investigation, regardless of whether they have or not been tried before.
...
- __Strive for easily reproducible tests:__ Anyone should be able to clone and run our tests with minimal effort. We have had good results using Docker for this, at least on Linux, as the overhead seems to be usually low and the results still representative.
